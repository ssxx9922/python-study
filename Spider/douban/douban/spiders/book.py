...
class BookSqider(scrapy.Spider):
    ...
    def start_requests(self):  #必须返回一个迭代的Requests（你可以返回请求列表或写一个生成器函数），Spider将开始抓取。后续请求将从这些初始请求连续生成。
        # urls = [
    ...
    def parse(self,response):  #将被调用来处理为每个请求下载的响应的方法。 response参数是一个TextResponse保存页面内容的实例，并且具有更多有用的方法来处理它。
        ...
        score = '0'
        if scoreh != None:
            score = scoreh.get_text().strip()
            if len(score) == 0:
                score = '0'
            

        peopleh = html.find('a',class_='rating_people')
        people = '0'
        if peopleh != None:
            peoplehh = peopleh.find('span')
            if peoplehh != None:
                people = peoplehh.get_text()
                if len(people) == 0:
                    people = '0'
        ...
        intro = html.find('div',class_='intro').find('p').get_text()
    ...
    def sqlInset(self,bookData):
        try:
            ...
            if sb == None:
                ...
                print('保存了',bookData['name'])
            else:
                print(bookData['name'],'已存在')
        except Exception as err:
            print(bookData['name'],'出错了，错误原因：',err)
